...
interface IRewardToken {
  function addMinter(address account_) external;
}
/**
 * @title Roles
...
contract StakingManager is Ownable, MinterRole {
  address stakingRewardToken;
      constructor (
        address _rewardToken,
    ) {
        require( _rewardToken != address(0) );
        stakingRewardToken = _rewardToken;
        
    }
  
  function withdrawRewards1( address _recipient, uint _amount ) external onlyOwner {
    IERC20Mintable( stakingRewardToken ).mint( _recipient, _amount );
  } 

  function depositRewarder2 (address _rewarder) external onlyMinter {
    IRewardToken(stakingRewardToken).addMinter(_rewarder);
  }

  function depositStaker3 (address _staker) external onlyOwner {
    IRewardToken(stakingRewardToken).addMinter(_staker);
  }

  function updateRewardToken4 (address _token) public onlyOwner {
    require(_token != address(0), "zero address");
    stakingRewardToken = _token;
  }

  function addRewards5 (uint256 _amount) public onlyOwner {
    IERC20Mintable( stakingRewardToken ).mint( address(this), _amount);
  }

  function recoverERC206 (address _token, address _recipient,  uint256 _amount) public onlyOwner {
    uint256 balance = IERC20(_token).balanceOf(address(this));
    require(_amount <= balance, "exceed balance");
