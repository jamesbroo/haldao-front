...
contract StakingManager is Ownable, MinterRole {
  ...
  function withdrawRewards( address _recipient, uint _amount ) external onlyOwner {
    IERC20Mintable( stakingRewardToken ).mint( _recipient, _amount );
  } 

  function depositRewarder (address _rewarder) external onlyMinter {
    IRewardToken(stakingRewardToken).addMinter(_rewarder);
  }

  function depositStaker (address _staker) external onlyOwner {
  ...
  function recoverERC20(address _token, address _recipient,  uint256 _amount) public onlyOwner {
